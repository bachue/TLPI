...
static void showTime(const struct timespec*, char*, size_t);
...
displayStatInfo(const struct stat *sb)
{
    ...
    showTime(&sb->st_atim, buf, 29);
    printf("Last file access:         %s\n", buf);
    showTime(&sb->st_mtim, buf, 29);
    printf("Last file modification:   %s\n", buf);
    showTime(&sb->st_ctim, buf, 29);
    printf("Last status change:       %s\n", buf);
}

void showTime(const struct timespec *ts, char *buf, size_t size) {
    char tmbuf[20];
    struct tm t;
    time_t sec = ts->tv_sec;
    long nsec = ts->tv_nsec;
    localtime_r(&sec, &t);
    strftime(tmbuf, 20, "%Y-%m-%d %H:%M:%S", &t);
    snprintf(buf, size, "%s.%09ld", tmbuf, nsec);
}
