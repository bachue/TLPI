...
int change_cap(int, int);
int add_cap(int);
int clear_cap(int);

int change_cap(int capability, int setting) {
    cap_t cap;
    cap = cap_get_proc();
    if (cap == NULL)
        return -1;
    if (cap_set_flag(cap, CAP_EFFECTIVE, 1, &capability, setting) == -1) {
        cap_free(cap);
        return -1;
    }
...
int add_cap(int capability) {
    return change_cap(capability, CAP_SET);
}

int clear_cap(int capability) {
    return change_cap(capability, CAP_CLEAR);
}
